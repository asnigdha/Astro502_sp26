...
def findfriends(targname,radial_velocity,velocity_limit=5.0,search_radius=25.0,rvcut=5.0,convergcut=5.0,radec=[None,None],output_directory = None,showplots=False,verbose=False,DoGALEX=False,DoWISE=False,DoROSAT=False):
    ...
    #     wr.writerow(['Gaia DR3','RA','DEC','Gmag','Bp-Rp','Voff(km/s)','Sep(deg)','3D(pc)','Vr(pred)','Vr(obs)','Vrerr','Plx(mas)','SpT','FnuvJ','W1-W3','RUWE','XCrate','RVsrc', 'PMRA', 'PMDec', 'PMRApred', 'PMDecpred'])
    #     for x in range(0 , np.array(zz).size):
    ...
    #                   "{0:.3f}".format(r['pmra'][yy[x]]), "{0:.3f}".format(r['pmdec'][yy[x]]), \
    #                   "{0:.3f}".format(Gpmrapmdec[yy[x],0]), "{0:.3f}".format(Gpmrapmdec[yy[x],1]), \
    #                   "{0:.3f}".format(gaiacoord.pm_ra_cosdec.value[yy[x]]), "{0:.3f}".format(gaiacoord.pm_dec.value[yy[x]]))
